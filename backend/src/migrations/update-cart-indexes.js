...
async function migrateCartIndexes() {
    try {
        ...
        const userCarts = await cartsCollection.countDocuments({
            userId: { $ne: null },
        });
        ...
        const invalidCarts = await cartsCollection.countDocuments({
            userId: null,
            sessionId: null,
        });
        ...
        console.log(
            `  Invalid carts (no userId or sessionId): ${invalidCarts}`,
        );
