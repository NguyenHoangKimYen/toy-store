...
const buildResultFromFeature = (feature, userInput = '') => {
  ...
  const cleanedHead = inputHead.replace(/(Phường|Xã|Thị Trấn|Quận|Huyện|Thành Phố).*/i, '').trim();
...
const vietMapRequest = async (endpoint, params = {}) => {
  const response = await axios.get(`https://maps.vietmap.vn/api/${endpoint}`, {
...
const verifyAddress = async (addressLine) => {
  try {
    ...
    const searchResults = await vietMapRequest('search', { text: addressLine });
    ...
    console.log('⚠️ VietMap search returned no result, trying autocomplete...');
    const suggestions = await vietMapRequest('autocomplete', { text: addressLine });
    ...
    if (fallback) {
      console.log(`Auto corrected: "${addressLine}" → "${fallback.formatted}"`);
      return {
        valid: true,
        userInput: addressLine,
    ...
    if (NODE_ENV === 'development') {
      return { valid: true, userInput: addressLine, formatted: addressLine, lat: null, lng: null, corrected: false };
    }

    return { valid: false, userInput: addressLine, formatted: null, lat: null, lng: null, corrected: false };
  } catch (error) {
...
const suggestAddress = async (keyword) => {
  try {
    if (!keyword || !VIETMAP_API_KEY) throw new Error('Missing keyword or API key');
    const suggestions = await vietMapRequest('autocomplete', { text: keyword });
