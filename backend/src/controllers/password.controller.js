const { forgotPasswordSchema, resetPasswordSchema } = require('../services/password.service.js');
...
const forgotPassword = async (req, res, next) => {
    try{
        ...
        const result = await passwordService.requestReset(value.emailOrPhoneOrUsername,{
            byEmail: (email) => userRepository.findByEmail(email, false),
            byPhone: (phone) => userRepository.findByPhone(phone, false),
            byUsername: (username) => userRepository.findByUsername(username, false),
        });
...
const resetPassword = async (req, res, next) => {
    try{
        ...
        await passwordService.resetPassword(value.userId, value.token, value.newPassword)
        res.json(
            {
                success: true,
