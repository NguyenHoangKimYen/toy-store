...
exports.calculateShippingFeeByUser = async (req, res) => {
  try {
    ...
    if (!address)
      return res.status(400).json({ success: false, message: 'User chưa có địa chỉ mặc định' });
    ...
    if ((!lat || !lng) && address.addressLine) {
      console.log(`Đang xác định tọa độ từ VietMap cho địa chỉ: ${address.addressLine}`);
    ...
    if (deliveryType === 'express' && !result.isExpressAllowed) {
      logExpressRejected({ userId, province: address.city, region: result.region });
    }
