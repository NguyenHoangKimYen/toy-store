...
const updateAddress = async (req, res, next) => {
    try {
        ...
        if (req.body.addressLine) {
            const { valid, formatted, lat, lng } = await verifyAddress(req.body.addressLine);
            if (!valid) {
                console.warn('VietMap xác minh thất bại cho:', req.body.addressLine);
                return res.status(400).json({
                    success: false,
...
const setDefaultAddress = async (req, res, next) => {
    try {
        ...
        if (!mongo.ObjectId.isValid(userId) || !mongo.ObjectId.isValid(addressId)) {
        ...
        if (!updated) {
            return res.status(404).json({ success: false, message: 'Failed to set default address' });
        }
...
const deleteAddress = async (req, res, next) => {
    try {
        ...
        if (!address) {
            return res.status(404).json({ success: false, message: 'Address not found or delete failed' });
        }
